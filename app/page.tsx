...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const distanceMiddle = middle! - 0; // distance from left edge of the screen to the middle element
    const distanceLeft = left! - 0; // distance from left edge of the screen to the middle element
    ...
    if(diamondDown.current){
      diamondDown.current.style.left = `calc(${distanceLeft}px - ${diamondDown.current.offsetWidth/2}px + 1px)`;
    }
    if(diamondUp.current){
      diamondUp.current.style.left = `calc(${distanceLeft}px - ${diamondUp.current.offsetWidth/2}px + 1px)`;
    }
  }, [screenSize]);
  ...
  return (
    <div className="w-full text-white relative overflow-hidden">
      <div className="relative z-0">
        ...
        <div ref={diamondUp} className="w-0 h-0 triangle absolute left-[6.5vw] top-[calc(256px-95px)] -z-1 hidden min-[1200px]:block"></div>
        <div ref={diamondDown} className="w-0 h-0 triangle rotate-180 absolute left-[6.5vw] top-[209px] -z-1 hidden min-[1200px]:block"></div>


        <div ref={horizontalLeft} className="bg-green w-[2px] h-[750px] absolute left-[8vw] top-64 -z-1 hidden min-[1200px]:block"></div>

        <div ref={bottomLeft} className="max-w-[41vw] w-full h-[100px] bottom_left absolute left-[8vw] top-[calc(256px+750px)] -z-1 hidden min-[1200px]:block"></div>
        <div
